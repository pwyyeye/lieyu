...
-(void)show{
    if (self.alertType==LYAlertTypeDefault) {
        //阴影部分高度
        ...
        if(_btn_cancel==nil){
            ...
            [_btn_cancel setTitleColor:COMMON_PURPLE forState:UIControlStateNormal];
        ...
        if(_btn_ok==nil){
            ...
            [_btn_ok setTitleColor:COMMON_PURPLE forState:UIControlStateNormal];
    ...
    }else if(self.alertType==LYAlertTypeUpToDown){
        CGFloat topviewHeight=SCREEN_HEIGHT*(1-_shade_proportion);
        UIView *TopView=[[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, topviewHeight)];
        TopView.backgroundColor=[UIColor whiteColor];
        
        _showView.frame=CGRectMake(0, 0, SCREEN_WIDTH, topviewHeight);
        [TopView addSubview:_showView];
        [self addSubview:TopView];
        
        _shadeButton=[[UIButton alloc] initWithFrame:CGRectMake(0, topviewHeight, SCREEN_WIDTH, (SCREEN_HEIGHT-topviewHeight))];
        
        [_shadeButton addTarget:self action:@selector(removeAlertView) forControlEvents:UIControlEventTouchUpInside];
        
        [self addSubview:_shadeButton];
        
        [self MoveView:self To:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT-64) During:0.3];
        _shadeButton.backgroundColor=[UIColor colorWithWhite:0 alpha:0.0];
    }
