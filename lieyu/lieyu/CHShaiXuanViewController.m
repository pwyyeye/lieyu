...
#import "BiaoQianBtn.h"
@interface CHShaiXuanViewController ()
{
...
@implementation CHShaiXuanViewController
...
- (IBAction)sureAct:(id)sender {
    ...
    for (int i=0;i< biaoqianList.count; i++) {
        NSDictionary *dic = biaoqianList[i];
        NSArray *arr=[dic objectForKey:@"data"];
        for (NSArray *arr1 in arr) {
            bool isflag=false;
            for (ProductCategoryModel *model in arr1) {
    ...
    //判断是否符合条件
    [self.delegate addShaiXuan:arrNew];
