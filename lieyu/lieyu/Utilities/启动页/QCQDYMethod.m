...
+ (CAAnimationGroup*)groupAnimations:(NSArray*)animations fillMode:(NSString*)fillMode forEffectLayer:(BOOL)forEffectLayer sublayersCount:(NSInteger)count{
	...
	if (forEffectLayer) {
		groupAnimation.duration = [QCQDYMethod maxDurationOfEffectAnimation:groupAnimation sublayersCount:count];
		}else{
		groupAnimation.duration = [QCQDYMethod maxDurationFromAnimations:animations];
	}
